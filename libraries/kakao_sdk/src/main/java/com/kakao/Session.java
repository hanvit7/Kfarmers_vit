...
public class Session {
     ...
     * 처음 세션을 접근할 때 사용한다.    <br/>
     * opened 상태 : 다음 acitivity로   <br/>
     * closed 상태 : 사용자 action을 받아 open 시도   <br/>
     * opening 상태 : 토큰 갱신 시도    <br/>
     ...
     */
    public static synchronized boolean initializeSession(final Context context, final SessionCallback sessionCallback, final AuthType... authTypes) {
        if (currentSession == null) {
            currentSession = new Session(context, authTypes);
        }
    ...
    public boolean implicitOpen(final SessionCallback sessionCallback) {
        if (currentSession.isOpening() && currentSession.accessToken.hasRefreshToken()) {
            internalOpen(sessionCallback, null);
            return true;
        } else {
            return false;
        }
     ...
     * {@link SessionState#OPENING} 상태이면 code 또는 refresh token 이용하여  access token 을 받아온다. 에러/취소시 {@link SessionState#CLOSED}, refresh 취소시에만 {@link SessionState#OPENING} 유지.   <br/>
     * param으로 받은 콜백으로 그 결과를 전달한다.  <br/>
     * @param authType 인증받을 타입. 예를 들어, 카카오톡 또는 카카오스토리 또는 직접 입력한 카카오계정
     */
    public void open(final AuthType authType) {
        internalOpen(null, authType);
    }
    ...
    private Session(final Context context, final AuthType... authTypes){
        if(context == null)
    ...
    /**
     * @param sessionCallback initialize시 설정한 sessionCallback을 그대로 사용하려고 할때는 지정하지 않을 수 있다.
     * @param authType  CLOSED 상태에서 authcode를 받아야 하는 시점에 필요.
     */
