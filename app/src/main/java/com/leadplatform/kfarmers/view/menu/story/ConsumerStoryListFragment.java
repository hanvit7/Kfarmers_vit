...
public class ConsumerStoryListFragment extends BaseRefreshMoreListFragment implements OnCloseShareDialogListener {
	...
	private ArrayList<StoryListJson> consumerStoryItemList;
	private FarmStoryListAdapter consumerStoryListAdapter;

	public static ConsumerStoryListFragment newInstance(String userIndex) {
		final ConsumerStoryListFragment f = new ConsumerStoryListFragment();
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (consumerStoryListAdapter == null) {
			consumerStoryItemList = new ArrayList<StoryListJson>();
			consumerStoryListAdapter = new FarmStoryListAdapter(getSherlockActivity(), R.layout.item_story, consumerStoryItemList, ((BaseFragmentActivity) getSherlockActivity()).imageLoader);
			setListAdapter(consumerStoryListAdapter);
			getConsumerStoryList(makeFarmStoryListData(true, 0));
		}
	}

	private void getConsumerStoryList(StoryListData data) {
		if (data == null)
			return;

		if (data.isInitFlag()) {
			bMoreFlag = false;
			consumerStoryListAdapter.clear();
			consumerStoryListAdapter.notifyDataSetChanged();
		}

		CenterController.getListConsumer(data, new CenterResponseListener(getSherlockActivity()) {
			@Override
					...
					case 0000:
						...
						if (root.findPath("List").isArray()) {
							...
							while (it.hasNext()) {
								...
								consumerStoryListAdapter.add(diary);
							}
	...
	private class FarmStoryListAdapter extends ArrayAdapter<StoryListJson> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				holder.dynamicViewBg = (DynamicImageViewBackground) convertView.findViewById(R.id.dynamicViewBg);
			...
			if (item != null) {
				...
				holder.rootLayout.setOnClickListener(new ViewOnClickListener() {
					...
					public void viewOnClick(View v) {
						...
						intent.putExtra("type", StoryDetailActivity.DETAIL_CONSUMER);
				...
				if (images.size() != 0) {
					holder.imageViewPager.setAdapter(new BaseSlideImageAdapter(getSherlockActivity(), item.DiaryIndex, StoryDetailActivity.DETAIL_CONSUMER, images, imageLoader));
					...
					holder.dynamicViewBg.setVisibility(View.VISIBLE);
				} else {
					holder.imageViewPager.setVisibility(View.GONE);
					holder.dynamicViewBg.setVisibility(View.GONE);
				}
			}
	...
	private OnLoadMoreListener loadMoreListener = new OnLoadMoreListener() {
		...
		public void onLoadMore() {
			if (bMoreFlag == true) {
				bMoreFlag = false;
				getConsumerStoryList(makeFarmStoryListData(false, Integer.valueOf(consumerStoryListAdapter.getItem(consumerStoryListAdapter.getCount() - 1).DiaryIndex)));
			} else {
				onLoadMoreComplete();
			}
		}
