...
public class InterviewStoryListFragment extends BaseRefreshMoreListFragment implements OnCloseShareDialogListener {
	public static final String TAG = "InterviewStoryListFragment";

	private boolean bMoreFlag = false;
	private String userIndex;
	private ArrayList<StoryListJson> interviewStoryItemList;
	private FarmStoryListAdapter interviewStoryListAdapter;

	public static InterviewStoryListFragment newInstance(String userIndex) {
		final InterviewStoryListFragment f = new InterviewStoryListFragment();
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (interviewStoryListAdapter == null) {
			interviewStoryItemList = new ArrayList<StoryListJson>();
			interviewStoryListAdapter = new FarmStoryListAdapter(getSherlockActivity(), R.layout.item_story, interviewStoryItemList, ((BaseFragmentActivity) getSherlockActivity()).imageLoader);
			setListAdapter(interviewStoryListAdapter);
			getinterviewStoryList(makeFarmStoryListData(true, 0));
		}
	}

	private void getinterviewStoryList(StoryListData data) {
		if (data == null)
			return;

		if (data.isInitFlag()) {
			bMoreFlag = false;
			interviewStoryListAdapter.clear();
			interviewStoryListAdapter.notifyDataSetChanged();
		}

		CenterController.getListInterview(data, new CenterResponseListener(getSherlockActivity()) {
			@Override
					...
					case 0000:
						...
						if (root.findPath("List").isArray()) {
							...
							while (it.hasNext()) {
								...
								interviewStoryListAdapter.add(diary);
							}
	...
	private class FarmStoryListAdapter extends ArrayAdapter<StoryListJson> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item != null) {
				holder.rootLayout.setTag(item.DiaryIndex);
				holder.rootLayout.setOnClickListener(new ViewOnClickListener() {
					...
					public void viewOnClick(View v) {
						...
						intent.putExtra("type", StoryDetailActivity.DETAIL_INTERVIEW);
				...
				if (images.size() != 0) {
					holder.imageViewPager.setAdapter(new BaseSlideImageAdapter(getSherlockActivity(), item.DiaryIndex, StoryDetailActivity.DETAIL_INTERVIEW, images, imageLoader));
	...
	private OnLoadMoreListener loadMoreListener = new OnLoadMoreListener() {
		...
		public void onLoadMore() {
			if (bMoreFlag == true) {
				bMoreFlag = false;
				getinterviewStoryList(makeFarmStoryListData(false, Integer.valueOf(interviewStoryListAdapter.getItem(interviewStoryListAdapter.getCount() - 1).DiaryIndex)));
			} else {
				onLoadMoreComplete();
			}
		}
