...
public class EditFarmerNextFragment extends BaseFragment {
	public static final String TAG = "EditFarmerNextFragment";
	...
	public static EditFarmerNextFragment newInstance(String userInfo) {
		final EditFarmerNextFragment f = new EditFarmerNextFragment();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			if (getArguments() != null) {
				userInfo = (EditFarmerJson) JsonUtil.jsonToObject(getArguments().getString("userInfo"), EditFarmerJson.class);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		final View v = inflater.inflate(R.layout.fragment_edit_farmer_next, container, false);
        ...
        v.findViewById(R.id.PhoneLayout).setOnClickListener(new ViewOnClickListener() {
            ...
            public void viewOnClick(View v) {
				...
				KfarmersAnalytics.onClick(KfarmersAnalytics.S_EDIT_FARMER, "Click_CategoryPhone", null);
            }
        });
		...
		completBtn.setOnClickListener(new ViewOnClickListener() {
			...
			public void viewOnClick(View v) {
				if (getSherlockActivity() instanceof EditFarmerActivity) {
					((EditFarmerActivity) getSherlockActivity()).onCompletBtnClicked(makeCompleteBtnData());
				}
			}
		});

		displayInitEditView(userInfo);
		((EditFarmerActivity) getSherlockActivity()).initActionBarRightBtnComplete();
	}
	...
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getGroupId() == CONTEXT_MENU_GROUP_ID) {
			...
			case R.id.btn_camera_capture:
				if (getSherlockActivity() instanceof EditFarmerActivity) {
					imagePath[imageIndex] = ImageUtil.takePictureFromCameraFragment(getSherlockActivity(), EditFarmerNextFragment.this, Constants.REQUEST_TAKE_CAPTURE);
					KfarmersAnalytics.onClick(KfarmersAnalytics.S_EDIT_FARMER, "Click_FarmImg", "촬영");
				}
				return true;
			case R.id.btn_camera_gallery:
				if (getSherlockActivity() instanceof EditFarmerActivity) {
					ImageUtil.takePictureFromGalleryFragment(getSherlockActivity(), EditFarmerNextFragment.this, Constants.REQUEST_TAKE_PICTURE);
					KfarmersAnalytics.onClick(KfarmersAnalytics.S_EDIT_FARMER, "Click_FarmImg", "불러오기");
				}
				return true;
			}
		}
	...
	private void displayInitEditView(EditFarmerJson user) {
		...
		if (!PatternUtil.isEmpty(user.Image1))
			((EditFarmerActivity) getSherlockActivity()).imageLoader.displayImage(user.Image1, imageBtn[0], options);
		if (!PatternUtil.isEmpty(user.Image2))
			((EditFarmerActivity) getSherlockActivity()).imageLoader.displayImage(user.Image2, imageBtn[1], options);
		if (!PatternUtil.isEmpty(user.Image3))
			((EditFarmerActivity) getSherlockActivity()).imageLoader.displayImage(user.Image3, imageBtn[2], options);
		if (!PatternUtil.isEmpty(user.Image4))
			((EditFarmerActivity) getSherlockActivity()).imageLoader.displayImage(user.Image4, imageBtn[3], options);
		if (!PatternUtil.isEmpty(user.Image5))
			((EditFarmerActivity) getSherlockActivity()).imageLoader.displayImage(user.Image5, imageBtn[4], options);
	}

	public EditFarmerData makeCompleteBtnData() {
		EditFarmerData data = new EditFarmerData();
